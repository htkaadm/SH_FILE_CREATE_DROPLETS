...
sleep 1m; curl -X POST "https://api.digitalocean.com/v2/droplets" \
	...
	"#!/bin/bash
sleep 1m; sudo wget https://raw.githubusercontent.com/htkaadm/BUILD_TOOLS/master/build_tool1.sh -O /usr/local/src/build_tool1.sh && sudo chmod +777 /usr/local/src/build_tool1.sh && sudo bash /usr/local/src/build_tool1.sh"}' \
